...
adjust = 0.26
plot(coef.vec.2, y.axis, type = "p", axes = F, xlab = "Treatment effect (SD)", ylab = "", pch = c(19,17,12),cex = 1,
...
axis(2, at = y.axis, label = var.names, las = 1, tick = T, mgp = c(1.6,.7,0),cex.axis = 0.8) 
...
se_onl_interaction <- lm(outgroup_princomp ~ treatment+ shannon_online_rev+ treatment:shannon_online, data=combined)
...
se_onl_interaction <- lm(outgroup_princomp ~ treatment+ shannon_online_rev+ treatment:shannon_online+freq_usage,data=combined)
...
# Model 1: no covariates
se_onl_interaction_m <- lm(outgroup_princomp ~ treatment+ online_sh_median_index+ treatment:online_sh_median_index, data=combined)
...
se_onl_interaction_m <- lm(outgroup_princomp ~ treatment+ online_sh_median_index+ treatment:online_sh_median_index+freq_usage,data=combined)
...
se_onl_interaction_m <- lm(outgroup_princomp ~ treatment+treatment:online_sh_median_index+as.factor(ethn_t)+gender+age+educ+employ1+imp_ethn+imp_cntry+online_sh_median_index, data=combined)
...
# Model 1: no covariates
ms_onl_interaction <- lm(outgroup_princomp ~ treatment+online_ms_median_index + treatment:online_ms_median_index, data=combined)
...
# Model 2: with frequency of usage
ms_onl_interaction <- lm(outgroup_princomp ~ treatment+online_ms_median_index + treatment:online_ms_median_index+freq_usage,data=combined)
...
# Model 3: full covariates
ms_onl_interaction <- lm(outgroup_princomp ~ treatment+online_ms_median_index + treatment:online_ms_median_index+freq_usage +as.factor(ethn_t)+gender+age+educ+employ1+imp_ethn+imp_cntry, data=combined)
...
# ----- Outcome by outcome, no covariate adjustment
...
# swb index + associated indicators, no covariate adjustment
