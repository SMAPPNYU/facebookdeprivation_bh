...
source("~/Desktop/facebookdeprivation_bh-master/scripts/01-analysis.R") 
...
for (i in 1:length(final_data$code)){
  final_data$fractionalization_offline_index[i] <- 1 - (final_data$bosniak[i]^2+final_data$serb[i]^2+final_data$croat[i]^2)}
...
# Calculated using the formula indicated in SI; already incorporated into the dataset;
for (i in 1:length(final_data$code)){
  final_data$shannon_offline[i] <- -(final_data$bosniak[i]*log(final_data$bosniak[i])+final_data$croat[i]*log(final_data$croat[i])+final_data$serb[i]*log(final_data$serb[i]))}

# For the localities where the share of one ethnic group is 0, calculated this value manually (otherwise gives NA):
final_data$shannon_offline[57] <- -(0.993*log(0.993))
final_data$shannon_offline[58] <- -(0.993*log(0.993))
final_data$shannon_offline[59] <- -(0.993*log(0.993))
final_data$shannon_offline[60] <- -(0.959*log(0.959)+0.005*log(0.005))
final_data$shannon_offline[61] <- -(0.959*log(0.959)+0.005*log(0.005))
final_data$shannon_offline[62] <- -(0.959*log(0.959)+0.005*log(0.005))
final_data$shannon_offline[63] <- -(0.959*log(0.959)+0.005*log(0.005))
final_data$shannon_offline[64] <- -(0.959*log(0.959)+0.005*log(0.005))
final_data$shannon_offline[91] <- -(0.995*log(0.995) + 0.001*log(0.001))
final_data$shannon_offline[151] <- -(0.247*log(0.247)+	0.749*log(0.749))
final_data$shannon_offline[300] <- -(0.806*log(0.806)+	0.016*log(0.016))

final_data$se_median_index <- NA
final_data$shannon_offline <- as.numeric(final_data$shannon_offline)
final_data$se_median_index <- ifelse(final_data$shannon_offline >=   median(final_data$shannon_offline), 0, 1)  # 1 is more homogenous
final_data$se_median_index 
