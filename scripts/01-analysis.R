...
setwd("/Users/nejlaasimovic/Desktop/facebookdeprivation_bh-master/data")
}

sessionInfo() 

# Source the analysis functions
source("~/Desktop/facebookdeprivation_bh-master/scripts/functions.R") 
...
bal.variables<-c("gender","age","educ","employ1","trust_media.1","freq_news","politics_int","numb_forums","time_usage","freq_usage","imp_ethn","imp_cntry","imp_relg", "imp_career" ,"imp_neigh","imp_hobby","imp_family")
...
balance_attr <- do.call(data.frame, # creating a table combining the mean values and p-values
                        list(TreatAttrition = apply(complete_data[,bal.variables][complete_data$treatment==1 & complete_data$attrition=="1",], 2, mean, na.rm=TRUE),
...
bal_attr_final <- do.call(data.frame, # creating a table combining the mean values and p-values
                 list(Attrition = apply(round(complete_data[,bal.variables][complete_data$attrition=="1",], digits=3), 2, mean, na.rm=TRUE),
                      FinalSample=  apply(round(complete_data[,bal.variables][complete_data$attrition=="0",], digits=3), 2, mean, na.rm=TRUE),
                      AttritionSD =  apply(round(complete_data[,bal.variables][complete_data$attrition=="1",], digits=3), 2, sd, na.rm=TRUE),
                      FinalSD =  apply(round(complete_data[,bal.variables][complete_data$attrition=="0",], digits=3), 2, sd, na.rm=TRUE),
                      t.test=p.values))
...
# ----------- Only the sample who sent their data [31 participants]
p.values0<-cbind(sapply(bal.variables, function (x) t.test(complete_data[1:384,][complete_data$treatment==0 & complete_data$attrition=="0",][,x],complete_data[1:384,][complete_data$treatment==0 & complete_data$attrition=="1",][,x]))[3,])
balance_attrition0 <- do.call(data.frame, # creating a table combining the mean values and p-values
                              list(CntrlParticipated = apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 &complete_data$attrition=="0",], 2, mean, na.rm=TRUE),
                                   CntrlParticipated_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 &complete_data$attrition=="0",], 2, sd, na.rm=TRUE),
                                   CntrlAttrition =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 & complete_data$attrition=="1",], 2, mean, na.rm=TRUE),
                                   CntrlAttrition_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 & complete_data$attrition=="1",], 2, sd, na.rm=TRUE),
                                   t.test=p.values0))
print(balance_attrition0, digits=4) # Control/Control Attrition

p.values0<-cbind(sapply(bal.variables, function (x) t.test(complete_data[1:384,][complete_data$treatment==1 & complete_data$attrition=="0",][,x],complete_data[1:384,][complete_data$treatment==1 & complete_data$attrition=="1",][,x]))[3,])
balance_attrition0 <- do.call(data.frame, # creating a table combining the mean values and p-values
                              list(TreatParticipated = apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 &complete_data$attrition=="0",], 2, mean, na.rm=TRUE),
                                   TreatParticipated_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 &complete_data$attrition=="0",], 2, sd, na.rm=TRUE),
                                   TreatAttrition =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 & complete_data$attrition=="1",], 2, mean, na.rm=TRUE),
                                   TreatAttrition_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 & complete_data$attrition=="1",], 2, sd, na.rm=TRUE),
                                   t.test=p.values0))
print(balance_attrition0, digits=4) # Treatment/Treatment Attrition


p.values0<-cbind(sapply(bal.variables, function (x) t.test(complete_data[1:384,][complete_data$treatment==1 & complete_data$attrition=="1",][,x],complete_data[1:384,][complete_data$treatment==0 & complete_data$attrition=="1",][,x]))[3,])
balance_attrition0 <- do.call(data.frame, # creating a table combining the mean values and p-values
                              list(CntrlParticipated = apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 &complete_data$attrition=="1",], 2, mean, na.rm=TRUE),
                                   CntrlParticipated_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==0 &complete_data$attrition=="1",], 2, sd, na.rm=TRUE),
                                   CntrlAttrition =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 & complete_data$attrition=="1",], 2, mean, na.rm=TRUE),
                                   CntrlAttrition_SD =  apply(complete_data[1:384,][,bal.variables][complete_data$treatment==1 & complete_data$attrition=="1",], 2, sd, na.rm=TRUE),
                                   t.test=p.values0))
print(balance_attrition0, digits=4) # Control/Treatment Drop-Out
...
# Table S7, Subjective Well Being: Adjusting for a full set of covariates
...
# For Table S8, treatment effect estimates and standard errors extracted from Table 6

# --------------------------------------
#  Robustness to Outliers: Table S9
# --------------------------------------
...
# These values form the last two columns of table S9; 
# the first two columns are just values extracted from table S5-S6 (for easier comparison)


# ------------- C.3.1 Attrition Robustness Check
sapply(bal.variables, function (i){
  mod <- lm(as.formula(paste0("attrition","~", i)), data=complete_data)
  results <- tidy(mod)
  results$outcome <- paste0(i)
  results$p.value <- results$p.value[2]
  print(results[c("p.value","outcome")])
})

complete_data$ethn_t <- NA
complete_data[which(complete_data$ethn=="1" | complete_data$ethn=="-1"),]$ethn_t <- 1 #Bosniak
complete_data[which(complete_data$ethn=="2"|complete_data$ethn=="-4"),]$ethn_t <- 2 #Serb
complete_data[which(complete_data$ethn=="3"|complete_data$ethn=="-5"),]$ethn_t <- 3 #Croat
complete_data[which(complete_data$ethn=="0"),]$ethn_t <- 4 #Bosnian
complete_data[which(complete_data$ethn=="-3"|complete_data$ethn=="-2"),]$ethn_t <- 5 #Other/Do not want to say

m1 <- glm(attrition~imp_hobby+imp_relg+educ+as.factor(ethn_t), data=complete_data, family="binomial")
final_data$prob <- NA
final_data$prob <- ifelse(is.na(final_data$prob), m1$fitted.values, NA) # predicted probabilities of attrition

TableC.3.a <- do.call(data.frame,rbind(
  reg_res_mod1_prob("c_news",final_data),
  reg_res_mod1_prob("swb",final_data),
  reg_res_mod1_prob("satisf",final_data),
  reg_res_mod1_prob("joy",final_data),
  reg_res_mod1_prob("fulf",final_data),
  reg_res_mod1_prob("nerv",final_data),
  reg_res_mod1_prob("boredom",final_data),
  reg_res_mod1_prob("loneliness",final_data),
  reg_res_mod1_prob("depression",final_data),
  reg_res_mod1_prob("isol",final_data)))


TableC.3.b <- do.call(data.frame,rbind(
  reg_res_mod2_prob("c_news",final_data),
  reg_res_mod2_prob("swb",final_data),
  reg_res_mod2_prob("satisf",final_data),
  reg_res_mod2_prob("joy",final_data),
  reg_res_mod2_prob("fulf",final_data),
  reg_res_mod2_prob("nerv",final_data),
  reg_res_mod2_prob("boredom",final_data),
  reg_res_mod2_prob("loneliness",final_data),
  reg_res_mod2_prob("depression",final_data),
  reg_res_mod2_prob("isol",final_data)))


TableC.3.c <- do.call(data.frame,rbind(
  reg_res_mod3_prob("c_news",final_data),
  reg_res_mod3_prob("swb",final_data),
  reg_res_mod3_prob("satisf",final_data),
  reg_res_mod3_prob("joy",final_data),
  reg_res_mod3_prob("fulf",final_data),
  reg_res_mod3_prob("nerv",final_data),
  reg_res_mod3_prob("boredom",final_data),
  reg_res_mod3_prob("loneliness",final_data),
  reg_res_mod3_prob("depression",final_data),
  reg_res_mod3_prob("isol",final_data)))

# --------*Table C.3.1* ------------
tableC.3.1 <- join_all(list(TableC.3.a, TableC.3.b, TableC.3.c), by="outcome") 
tableC.3.1
# -------- *Table C.3.1* ------------
...
# ---------  Table C.3.1
TableC.3.a <- do.call(data.frame,rbind(
  reg_res_mod1_prob("ft",final_data),
  reg_res_mod1_prob("sd",final_data),
  reg_res_mod1_prob("multi_stat",final_data),
  reg_res_mod1_prob("inchrct_final_data",final_data),
  reg_res_mod1_prob("othchrct_final_data",final_data),
  reg_res_mod1_prob("outgroup_index",final_data),
  reg_res_mod1_prob("outgroup_princomp",final_data)))

TableC.3.b <- do.call(data.frame,rbind(
  reg_res_mod2_prob("ft",final_data),
  reg_res_mod2_prob("sd",final_data),
  reg_res_mod2_prob("multi_stat",final_data),
  reg_res_mod2_prob("inchrct_final_data",final_data),
  reg_res_mod2_prob("othchrct_final_data",final_data),
  reg_res_mod2_prob("outgroup_index",final_data),
  reg_res_mod2_prob("outgroup_princomp",final_data)))

TableC.3.c <- do.call(data.frame,rbind(
  reg_res_mod3_prob("ft",final_data),
  reg_res_mod3_prob("sd",final_data),
  reg_res_mod3_prob("multi_stat",final_data),
  reg_res_mod3_prob("inchrct_final_data",final_data),
  reg_res_mod3_prob("othchrct_final_data",final_data),
  reg_res_mod3_prob("outgroup_index",final_data),
  reg_res_mod3_prob("outgroup_princomp",final_data)))


# -------- *Table C.3.1* ------------
tableC.3.1 <- join_all(list(TableC.3.a, TableC.3.b, TableC.3.c), by="outcome")
print(tableC.3.1, digits=3)
# -------- *Table C.3.1* ------------
